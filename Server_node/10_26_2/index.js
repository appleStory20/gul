...
io.sockets.on('connection', function (socket) {
  ...
  socket.on("STA", function(msg){
  	...
  	// SWI 명령 전송
   	 ...
   	 console.log( msg);
	 serial_list.push(msg.account);
	 console.log("1 "+msg.account);
	 if(client_list[msg.account] == null){
		client_list[msg.account] = new client(socket, msg.account, msg.name);
		// 현재 msg.account 은 어플의 account 값 이 값을 이용하여 db를 검색하고 학년과  같은 값을 추출
		// 하여 client_list 에 저장 , room info에도 학년 값을 넣을 것
		console.log(client_list);
	 }
	 else {
		reRegistration(msg.account, msg.name, socket);
 ...
 // socket.emit("account", JSON.stringify(json.getJsonData(1)));
...
//	  console.log(account);
	  ...
	  // id를 받아온 후 함수 호출을 통해 랜덤 알고리즘 수행
	 randomMatching(account);
  ...
  socket.on("RCS", function(msg){
  	// 매칭 실행
  	// 매칭 되면 RSM 명령 전송
  	matchingRandom(socket, msg.account);
  });
...
function randomMatching( account){
	  ...
	  db.query("SELECT * FROM user WHERE account = ?;" , [account] , function(err, rows, fields){
		...
		// sql에서 뽑은 값을 서버에 저장 
		  for(var i =0; i<rows.length; i++){
			  ...
			  c_groupnum = rows[i].groupnum;
		  }
		  db.query("SELECT * FROM groupinfo;", function(err, rows, fields){
			  
			  if(!err){
				  for( var j=0; j<rows.length; j++){
					  c_groupnum = row[i].groupnum;
				  }
				  if(c_groupnum = null){
					  room_info( roomNum, account, '', '', '', 
